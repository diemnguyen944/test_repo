...
The Minimum Viable Product (MVP) for "Moneta for Enterprise" will enable a core end-to-end flow using a pre-paid funding model: a Corporate Administrator can be approved by an MO, fund the corporate account, onboard users, set a single company-wide policy for access and spending, and receive a consolidated bill.

**MVP Includes:**

- **Manual Corporate Onboarding:** A corporate application process that requires manual review and approval by an MO Admin.
- **Core User Management & Groups:** The ability for an Admin to invite individual employees, invite users in bulk via file upload, and organize them into User Groups.
- **Hierarchical Policy Controls:** The ability for an Admin to set spending limits and service access policies at the company, group, and individual levels, with specific rules overriding general ones.
- **Post-Paid Funding Model:** The core financial model will be based on a credit limit assigned by the MO.
- **Transaction Tracking & Billing:** Real-time transaction logs for both Admins and Employees, and a consolidated monthly statement for the Admin.
...
- Autonomous Corporate Verification: The onboarding process requires manual MO Admin approval.
- Advanced Role-Based Access Control (RBAC): The MVP supports only "Administrator" and "Employee" roles. More complex roles are post-MVP.
- Pre-Paid (Top-Up) Funding Model: The MVP will focus exclusively on the Post-Paid model.
- Advanced Reporting & Analytics: The MVP provides transaction logs and a basic billing report, but a full analytics dashboard with data visualization is post-MVP.

### 2.2 Epic Summary

| Epic | Description | Priority |
|------|-------------|----------|
| **Corporate mPass Onboarding** | Covers the application, approval, and initial setup for a corporate entity. | Must |
...
#### 2.3.1 Epic 1: Corporate mPass Onboarding
...
| **US-1.4** | As an **MO Admin**, I want to assign a master spending limit to a Corporate mPass so that **we can manage our collection risk for corporate clients**. | M |
...
| **US-1.7** | As an **MO Admin**, I want to suspend or resume a corporate mPass so that **I can immediately revoke access and prevent further spending from a corporate**. | M |

#### 2.3.2 Epic 2: User Provisioning

| ID | User Story | Priority |
|----|------------|----------|
| **US-2.1** | As a **Corporate Administrator**, I want to invite an employee via email to create a Corporate mPass so that **I can grant them access to company-funded digital services**. | H |
| **US-2.2** | As a **Corporate Administrator**, I want to upload a file to **invite users in bulk** so that **I can efficiently onboard an entire team or department at once**. | M |
| **US-2.3** | As a **Corporate Employee**, I want to accept an email invitation and register for my Corporate mPass so that **I can start accessing the digital services my company provides**. | H |
| **US-2.4** | As a **Corporate Administrator**, I want to view a list of all my employees and their activation status so that **I can track who has successfully onboarded**. | H |
| **US-2.5** | As a **Corporate Administrator**, I want to suspend or de-provision an employee's mPass so that **I can immediately revoke access and prevent further spending when an employee leaves the company**. | H |

#### 2.3.3 Epic 3: User Group Management

| ID | User Story | Priority |
|----|------------|----------|
| **US-3.1** | As a **Corporate Administrator**, I want to create, name, and delete **User Groups (Company Department)** so that **I can organize my employees for efficient policy and budget management**. | M |
| **US-3.2** | As a **Corporate Administrator**, I want to assign an existing user to a **User Group** from the user list so that **they inherit the policies and budgets associated with that group**. | M |
| **US-3.3** | As a **Corporate Administrator**, I want to remove a user from a **User Group** so that **they revert to the company's default policies**. | M |
| **US-3.4** | As a **Corporate Administrator**, I want to view a list of all my employees, sortable by their **assigned User Group**, so that **I can easily see how my team is organized**. | M |
...
| **US-4.4** | As a **Corporate Administrator**, I want to set a company-wide service access policy by whitelisting or blacklisting Publishers and categories so that I can ensure all employees adhere to a baseline of approved services. | M |
...
| **US-4.7** | As a **Corporate Employee**, I want to be notified if an attempted purchase is denied due to a policy restriction so that I understand why the transaction failed. | M |
| **US-4.8** | As a **Corporate Employee**, I want to browse the mPass Guide and only see services my active policy permits so that I can easily find the tools I am authorized to use. | M |
| **US-4.9** | As an **MO**, I must deny any "Subscription Charge Authorization" request that violates any rules defined in the active policy so that **the company is protected from exceeding its approved budgets**. | H |
| **US-4.10** | As a **Corporate Employee**, I want to browse the mPass Guide and only see services my active policy permits so that **I can easily find and use the tools I am authorized to access**. | H |
...
| **US-5.2** | As a **Corporate Administrator**, I want to filter and sort the company's transaction log by employee, User Group, and date so that I can easily analyze spending patterns and prepare internal reports. | H |
| **US-5.3** | As a **Corporate Administrator**, I want to receive a *single, consolidated monthly statement* from my MO so that I have a simple bill to process. | H |
| **US-5.4** | As a **Corporate Administrator**, I want to pay my single monthly statement in full to my MO so that my company's account remains in good standing. | H |
| **US-5.5** | As a **Corporate Administrator**, I want to view a detailed, itemized report broken down by **User Group** and by **individual employees** so that I can analyze spending. | H |

#### 2.3.6 Epic 6: Corporate Dispute Management

| ID | User Story | Priority |
|----|------------|----------|
| **US-6.1** | As a **Corporate Employee**, I want to dispute a transaction from my history so that I can get an immediate credit for a charge I believe is incorrect. | H |
| **US-6.2** | As a **Corporate Administrator**, I want to dispute a transaction directly from the company-wide transaction log so that I can correct billing errors on behalf of the company. | H |
| **US-6.3** | As a **MO**, when a dispute is initiated by either an Employee or an Administrator, I must automatically process a credit for the Corporate Master Account so that the "customer-first" policy is upheld. | H |
| **US-6.4** | As a **Corporate Administrator**, I want to view a real-time log of all disputes and their automatic compensations so that I have full visibility into dispute activity and its financial impact. | H |
| **US-6.5** | As a **MO**, I must apply a "Corporate Trust Score" (CTS) to the Corporate mPass to mitigate abuse so that **the network is protected from fraudulent dispute activity**. | H |
| **US-6.6** | As a **Corporate Administrator**, I want to see all dispute-related credits clearly itemized on my consolidated monthly statement so that our billing is accurate and transparent. | H |
